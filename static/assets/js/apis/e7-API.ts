...
async function fetchE7Data(url: string): Promise<unknown | null> {
	try {
...
type artifactRawList = Array<{code: string, name: string}>

async function fetchArtifactJSON(lang: string | null  = null): Promise<artifactRawList | null> {
	console.log(`Fetching hero data (lang=${lang ?? "all"}) from E7 Server...`);
	let data: unknown = await fetchE7Data(ARTIFACT_URL);
	let output: null | artifactRawList = null;
	if (data && lang && typeof data === "object" && lang in data) {
		let record = data as Record<string, artifactRawList>;
...
async function fetchUserJSON(world_code: string): Promise<unknown | null> {
...
async function fetchInfo(uid: string, worldCode: string, lang = "en"): Promise<unknown | null> {
