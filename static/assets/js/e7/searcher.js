import { getStrMatches } from "../utils";
        ...
        if (!this.DOMAIN_CACHE[domain]) {
            switch (domain) {
                case SEARCH_DOMAINS.GLOBAL_SERVER:
                    this.DOMAIN_CACHE[domain] = UserManager.getUserMap(WORLD_CODE_ENUM.GLOBAL);
                    break;
                case SEARCH_DOMAINS.KOR_SERVER:
                    this.DOMAIN_CACHE[domain] = UserManager.getUserMap(WORLD_CODE_ENUM.KOR);
                    break;
                case SEARCH_DOMAINS.JPN_SERVER:
                    this.DOMAIN_CACHE[domain] = UserManager.getUserMap(WORLD_CODE_ENUM.JPN);
                    break;
                case SEARCH_DOMAINS.ASIA_SERVER:
                    this.DOMAIN_CACHE[domain] = UserManager.getUserMap(WORLD_CODE_ENUM.ASIA);
                    break;
                case SEARCH_DOMAINS.EU_SERVER:
                    this.DOMAIN_CACHE[domain] = UserManager.getUserMap(WORLD_CODE_ENUM.EU);
                    break;
