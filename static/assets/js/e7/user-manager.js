...
async function getUserMapFromE7Server(world_code) {
    ...
    return Object.fromEntries(rawUserJSON.users.map(user => [user.nick_no, createUser(user, world_code)]));
}
...
let UserManager = {
    ...
    findUser: async function(userData) {
        ...
        if (userData.id) {
            for (const world_code of WORLD_CODES) {
                ...
                const user = Object.values(userMap).find(user => user.id === userData.id);
                if (user) {
                    console.log(`Found user: ${JSON.stringify(user)} in world code: ${world_code}`);
                    return { user, error: false };
                } else {
                    console.log(`Could not find user with ID: ${userData.id} in world code: ${world_code} from client-side means`);
                }
            }
        }
        ...
        // try to find user by name and world code
            ...
            const user = Object.values(userMap).find(user => lowerCaseName === user.name.toLowerCase());
            if (user) {
                console.log(`Found user: ${JSON.stringify(user)} in world code: ${world_code}`);
                return { user, error: false };
            } else {
            ...
            return { user: null, error: "Must pass a user object with either user.name and user.world_code or user.id to fetch user" };
        }
    ...
    addP2ServersByID : async function(battles) {
        ...
        let userMaps = [];
        try {
            userMaps = await Promise.all([...WORLD_CODES].map(world_code => this.getUserMap(world_code)));
        } catch (error) {
            console.error(error);
        ...
        battleList.forEach(battle => {
            const world_code = userMaps.find(userMap => userMap[battle["P2 ID"]])?.[battle["P2 ID"]].world_code;
            if (world_code) {
                battle["P2 Server"] = this.convertServerStr(world_code);
            } else {
                console.log(`Could not find user with ID: ${battle["P2 ID"]} in client-side means ; typeof: ${typeof battle["P2 ID"]}, id type: ${typeof Object.keys(USER_MAPS[0])[0]}`);
