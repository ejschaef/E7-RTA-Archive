...
const cleanStr = (world_code: string) => WORLD_CODE_TO_CLEAN_STR[world_code];

function findUser(userData: string | number, users: User[], dataExtractFn: (user: User) => string | number) {
...
async function findUserClientSide(user: UserSearchData, userWorldCode: string): Promise<{ user: User | null; ok: boolean }> {
	...
	let userData: string | number, dataExtractFn: (user: User) => string | number;
	if (user.id) {
		userData = user.id;
...
let UserManager = {
	 ...
	 */
	findUser: async function (searchUser: UserSearchData): Promise<User> {
	...
	setUser: async function (user: User): Promise<void> {
	...
	clearUserDataLists: async function (): Promise<void> {
